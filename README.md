...
Checklist templates provide practical, career-specific guidance for banking,
transport, aged care, disability work, childcare and construction. Each item can
include the responsible provider, official application URL, eligibility notes and
documents to prepare, including screening checks, White Cards, high-risk work
licences, TFN and superannuation setup. Requirements vary by state, role,
employer and visa, so users should confirm current conditions with the linked
official authority.
...
`PATCH /checklist/tasks/{taskId}` updates the linked records in both directions
without allowing access to another user's roadmap. Preview data should be kept in
the client's local storage until the user chooses to sign up; persisted profile
and roadmap records are scoped by the authenticated user.
